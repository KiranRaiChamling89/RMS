<?php
    ...
    if(isset($name)) {
        $selectQuery = "SELECT id FROM rms_types WHERE name = ?";
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_stmt_num_rows($stmt) > 0){
                $_SESSION['error'] = "Food Type: ".$name." exists. Duplicate entry is not allowed!";
            } else {
                $currentDT = date("Ymdhis");
                $insertQuery = "INSERT INTO rms_types (name, status, created_at) VALUES (?, ?, ?)";
